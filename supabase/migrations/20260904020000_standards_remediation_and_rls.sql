...
-- Remediate only unassessed claim_evidence rows lacking citation locator & excerpt
UPDATE public.claim_evidence ce
SET evidence_strength = NULL
WHERE ce.evidence_strength = 'direct conclusive'
  AND ce.citation_locator IS NULL
  AND ce.supporting_excerpt IS NULL;

UPDATE public.claim_evidence ce
SET directness = NULL
WHERE ce.directness = 'direct'
  AND ce.citation_locator IS NULL
  AND ce.supporting_excerpt IS NULL;
