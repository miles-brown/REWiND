/**
 * REWIND EVIDENCE ATLAS — SEED SCRIPT: GROUP 1 PEOPLE
 *
 * Populates PostgreSQL / Supabase with the first group of 40 canonical public figures
 * and their associated aliases and coverage programmes.
 */
...
async function main() {
  ...
  if (!connectionString) {
    console.warn("⚠️ DATABASE_URL is not set. Skipping live Supabase DB population. Local store remains populated.");
    return;
  }
  ...
  try {
    ...
    console.log(`👤 2. Ingesting ${group1PeopleSeed.length} Public Figure Records...`);
    for (const p of group1PeopleSeed) {
    ...
    console.log("✅ Group 1 Public Figures (40 Records) Successfully Ingested!");
  } catch (err) {
