...
import { ArrowRight, ChevronLeft, ChevronRight, CirclePause, CirclePlay, ExternalLink, Gauge, MapPin } from "lucide-react";
...
export function PersonTimeline({person,records}:{person:Person;records:EventRecord[]}){
  ...
  useEffect(()=>{if(!playing||ordered.length<2)return;const timer=setInterval(()=>setIndex(i=>i>=ordered.length-1?0:i+1),speed);return()=>clearInterval(timer)},[playing,speed,ordered.length]);
  ...
  const choose=(id:string)=>{const next=ordered.findIndex(record=>record.id===id);if(next>=0)setIndex(next)};
  ...
  return <section className="person-time-machine" aria-label={`${person.name} chronological timeline`}>
    ...
    <div className="person-time-console">
      ...
      <div className="play-controls"><button onClick={()=>setIndex(i=>Math.max(0,i-1))} disabled={safeIndex===0} aria-label="Previous event"><ChevronLeft/></button><button className="main-play" onClick={()=>setPlaying(value=>!value)} aria-label={playing?"Pause timeline":"Play timeline"}>{playing?<CirclePause/>:<CirclePlay/>}</button><button onClick={()=>setIndex(i=>Math.min(ordered.length-1,i+1))} disabled={safeIndex===ordered.length-1} aria-label="Next event"><ChevronRight/></button></div>
      <div className="slider-wrap"><Slider aria-label={`${person.name} timeline position`} min={0} max={Math.max(0,ordered.length-1)} step={1} value={[safeIndex]} onValueChange={value=>{setPlaying(false);setIndex(value[0])}}/><div><span>{ordered[0].startDate.slice(0,4)}</span><b>{event.startDate.slice(0,4)}</b><span>{ordered.at(-1)?.startDate.slice(0,4)}</span></div></div>
